...
const processMovie = async (tmdbMovie) => {
  try {
    ...
    if (streams.length) {
      await db.replaceStreams('movie', movieId, streams);
    }
...
const processSeries = async (tmdbSerie) => {
  try {
    ...
    // Solo guarda metadata de episodios — SIN cachear streams
    // Los streams se cargan on-demand cuando el usuario los pide
